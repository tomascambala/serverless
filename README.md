...
* `dueDate` (string) - date and time by which an item is completed
...
* `Auth` - this function implement a custom authorizer for API Gateway that is added to all other functions.
...
The `client` folder contains a web application that can use the API that be developed in the project.
...
This start a development server with the React application that will interact with the serverless TODO application.